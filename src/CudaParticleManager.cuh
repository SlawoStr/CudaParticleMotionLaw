...
{
public:
	CudaParticleManager(float particleSpeed = 0.0f, float alpha = 0.0f, float beta = 0.0f, float reactRadius = 0.0f, float2 simulationBound = float2{ 0.0f,0.0f }, int cpuThreadNumber = 1, int threadPerBlock = 1);
...
private:
	...
	bool								m_cpuMode{ false };		//!< Update mode GPU or CPU
};
